...
class Prioritized_ERB:
    ...
    def store(self, state ,  action ,  reward ,  done ,  next_state ):
        ...
        if self.items_cnt > self.memory_size:
            ...
            if old_item.priority == self.p_max:
                p_list = [i[1].priority for i in self.data_buffer.items()]
                p_list.pop(index)
                self.p_max = max(p_list)

            if old_item.weight == self.w_max:
                w_list = [i[1].weight for i in self.data_buffer.items()]
                w_list.pop(index)
    ...
    def sample(self,batch_size):
        indices =random.sample(range(self.items_cnt), batch_size)
        ...
        w = [val.probability for val in list_data]
        batch = random.choices(self.data_buffer,weights=w,k=batch_size)

        # Get indices and weights associated
        indices =[item.index for item in batch] #tuple([item.index for item in batch])
