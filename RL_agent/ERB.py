...
class UniformReplayBuffer():
    ...
    def __init__(self, capacity: int,   # the capacity of the buffer
        ...
        self.buffer = {"observation":  np.zeros((capacity,*o_shape),dtype=np.float32),
                       ...
                       "next_state": np.zeros((capacity,*s_shape),dtype=np.float32),
                        "exp_action":     np.zeros((capacity,*a_shape),dtype=np.float32),
}
    ...
    def store(self, o: np.ndarray,
        ...
        self.buffer["exp_action"][self.idx] = aE if type(aE) == int else aE.copy()
    ...
    def sample(self, n: int):
        ...
        exp_actions =    self.buffer["exp_action"][idxes]


        return (observations,states,actions,rewards,dones,next_observations,new_states,idxes,w,exp_actions)
...
class PrioritizedReplayBuffer(UniformReplayBuffer):
    ...
    def store(self, o: np.ndarray, 
                    ...
                    sp: np.ndarray,
                    aE:np.ndarray):
        super().store(o,s,a,r,d,op,sp,aE)
        self.cnt+=1
